...
async function main() {
  ...
  const ConfiguracionAfip = require(path.join(__dirname, '..', 'server', 'models', 'ConfiguracionAfip'));
  const Vencimiento = require(path.join(__dirname, '..', 'server', 'models', 'Vencimiento'));
  ...
  const testUser = new User({
    googleUid: 'test-google-uid-2',
    email: 'test2@example.com',
    emailNotificaciones: 'test2@example.com',
